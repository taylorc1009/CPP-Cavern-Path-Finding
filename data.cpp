...
void readCAV(char* name, std::vector<std::shared_ptr<Cavern>> *caverns) {
	std::string path(std::string(name) + std::string(".cav")); //builds the path to the .cav file - C++ cannot concatenate pure strings (name + ".cav") as, to the compiler, they're initially pointers
